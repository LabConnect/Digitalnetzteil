...
Aus Sicht des Masters läuft der Zugrif auf den Buffer genau wie bei einem
I2C-EEPROm ab.
...
// Je nach Statuscode in TWSR müssen verschiedene Bitmuster in TWCR geschreiben
// werden(siehe Tabellen im Datenblatt!).
...
/*ISR, die bei einem Ereignis auf dem Bus ausgelöst wird. Im Register TWSR
befindet sich dann
...
ISR(TWI_vect) {
  ...
  case TW_SR_DATA_ACK: // 0x80 Slave Receiver, ein Datenbyte wurde empfangen
    ...
    if (buffer_adr == 0xFF) // erster Zugriff, Bufferposition setzen
    {
      ...
      TWCR_ACK; // nächstes Datenbyte empfangen, ACK danach, um nächstes Byte
                // anzufordern
    } else // weiterer Zugriff, nachdem die Position im Buffer gesetzt wurde.
           // NUn die Daten empfangen und speichern
    {
  ...
  case TW_ST_SLA_ACK: // 0xA8 Slave wurde im Lesemodus adressiert und hat ein
                      // ACK zurückgegeben.
  ...
  case TW_ST_LAST_DATA: // 0xC8  Last data byte in TWDR has been transmitted
